#ifndef SHA224_SHA244CRYPTOSERVICEPROVIDER_H
...
private:
    ...
    void InitialState(uint32_t H[]);
    static uint32_t CircularRightRotate(uint32_t x, uint32_t n);
    ...
    void ProcessBlock();
    void PadInputMessage();
    void ToBigEndian(uint8_t * hash);
    void Update(const std::string &data);
    std::string FinalProcess();
    void PadTotalInputMessageLength();
};


#endif //SHA224_SHA244CRYPTOSERVICEPROVIDER_H
